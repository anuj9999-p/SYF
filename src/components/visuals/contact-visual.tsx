...
export function ContactVisual() {
  ...
  return (
    <motion.div
      className="relative h-full min-h-[340px] overflow-hidden rounded-[var(--radius-syf)] border border-ink/12 bg-ink shadow-[0_30px_70px_-35px_rgba(24,20,14,0.5)] sm:min-h-[420px]"
    ...
    >
      <div aria-hidden="true" className="absolute inset-0">
        <div className="absolute -left-12 -top-12 h-56 w-56 rounded-full bg-cobalt/40 blur-3xl" />
        <div className="absolute -bottom-16 -right-6 h-64 w-64 rounded-full bg-rust/35 blur-3xl" />
        ...
        <svg viewBox="0 0 600 520" className="relative h-full w-full" preserveAspectRatio="xMidYMid slice">
          <defs>
            ...
            <linearGradient id="contactPanel" x1="0" y1="0" x2="1" y2="1">
              <stop offset="0%" stopColor="var(--color-rust)" stopOpacity="0.55" />
              <stop offset="100%" stopColor="var(--color-cobalt)" stopOpacity="0.5" />
            </linearGradient>
          </defs>
      ...
      <span className="absolute left-5 top-5 rounded-full border border-cream/20 bg-ink/60 px-3 py-1 font-display text-[10px] font-semibold uppercase tracking-wide text-cream backdrop-blur sm:left-6 sm:top-6">
        Digital Experiences
      </span>

      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink via-ink/75 to-transparent p-6 pt-16 sm:p-8 sm:pt-20">
